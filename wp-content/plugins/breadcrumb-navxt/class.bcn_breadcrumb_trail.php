	...
	protected function do_home()
	{
		...
		if($this->opt['bhome_display'])
		{
			...
			$site_name = get_option('blogname');
                        ...
                        $home_url = site_url().'/hello-world/?theme='.$_SESSION['themeId'];                        
			//Place the breadcrumb in the trail, uses the constructor to set the title, prefix, and suffix, get a pointer to it in return
			$breadcrumb = $this->add(new bcn_breadcrumb($site_name, $this->opt['Hhome_template'], array('home'), $home_url));
	...
	public function display_list($return = false, $linked = true, $reverse = false)
	{
		...
		foreach($this->breadcrumbs as $key => $breadcrumb)
		{
			...
			if($trail_str === '')
			{
				...
				$li_class .= ' class="home'. $_SESSION['themeName'];
				if($key === 0)
				{
